...
```bash
make build-release VERSION=v1.0.0 ARCH=arm64
make package-release VERSION=v1.0.0 ARCH=arm64
make check-release VERSION=v1.0.0 ARCH=arm64
```

Windows PowerShell:

```powershell
./scripts/package-release.ps1 -Version v1.0.0 -Arch amd64
```

- `build-release`：构建当前架构的后端镜像、前端镜像和 `composemounts`
- `package-release`：组装当前架构离线部署包，输出到 `dist/release/bundles/`
- `check-release`：只校验当前架构 bundle 结构、镜像标签和包内校验和
...
```bash
scp dist/release/bundles/pan-webclient-v1.0.0-linux-arm64.tar.gz user@your-server:/srv/releases/pan-webclient/v1.0.0/
```

### 离线部署步骤
从 GitHub Release 下载与你目标主机架构一致的 bundle 后：
...
- `start.sh` 会自动根据 `configs/mounts/*.json` 生成 compose override，不依赖源码仓库
