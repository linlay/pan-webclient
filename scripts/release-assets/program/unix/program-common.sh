...
MOUNTS_DIR="$CONFIG_DIR/mounts"
DATA_DIR="$BUNDLE_ROOT/data"
RUN_DIR="$BUNDLE_ROOT/run"
PID_FILE="$RUN_DIR/$PROGRAM_NAME.pid"
LOG_FILE="$RUN_DIR/$PROGRAM_NAME.log"
...
program_validate_bundle() {
  ...
  program_require_file "$CONFIG_DIR/local-public-key.example.pem"
  program_require_dir "$MOUNTS_DIR"
...
program_load_env() {
  ...
  APP_AUTH_LOCAL_PUBLIC_KEY_FILE="${APP_AUTH_LOCAL_PUBLIC_KEY_FILE:-./configs/local-public-key.pem}"
  PAN_DATA_DIR="${PAN_DATA_DIR:-./data}"
  export API_PORT FRONTEND_DIST_DIR APP_AUTH_LOCAL_PUBLIC_KEY_FILE PAN_DATA_DIR
}

program_prepare_runtime_dirs() {
  mkdir -p "$DATA_DIR" "$RUN_DIR" "$MOUNTS_DIR"
}
...
program_clear_stale_pid() {
  ...
  if [[ -n "$pid" ]] && kill -0 "$pid" >/dev/null 2>&1; then
    program_die "$PROGRAM_NAME is already running with pid $pid"
  fi
  rm -f "$PID_FILE"
}
...
program_start_backend_daemon() {
  ...
  echo "[program-start] started $PROGRAM_NAME in daemon mode (pid=$pid)"
...
program_stop_backend() {
  ...
  for _ in $(seq 1 30); do
    if ! kill -0 "$pid" >/dev/null 2>&1; then
      rm -f "$PID_FILE"
      echo "[program-stop] stopped $PROGRAM_NAME (pid=$pid)"
      return
    fi
    sleep 1
  done
